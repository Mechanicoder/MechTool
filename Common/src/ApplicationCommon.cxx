...
void ApplicationCommonWindow::createStandardOperations()
{
    ...
    QAction *fileImportAction = new QAction(tr("Import"), this);
    fileImportAction->setToolTip(tr("Import"));
    fileImportAction->setStatusTip(tr("Import"));
    connect(fileImportAction, SIGNAL(triggered()), this, SLOT(onImportFile()));
    ...
    myFilePopup->addAction(fileImportAction); // 导入文件
...
void ApplicationCommonWindow::createCasCadeOperations()
{
    ...
    // 分析曲面法向
    a = new QAction(QPixmap(dir + QObject::tr("ICON_TOOL_ANALYSENORMAL")), QObject::tr("MNU_TOOL_ANALYSENORMAL"), this);
    a->setToolTip(QObject::tr("TBR_TOOL_ANALYSENORMAL"));
    a->setStatusTip(QObject::tr("TBR_TOOL_ANALYSENORMAL"));
    connect(a, SIGNAL(triggered()), this, SLOT(onToolAction()));
    myToolActions.insert(ToolAnalyseNormalId, a);

    a = new QAction(QPixmap(dir + QObject::tr("ICON_TOOL_SURFACEOFFSET")), QObject::tr("MENU_TOOL_SURFACEOFFSET"), this);
    a->setToolTip(QObject::tr("TBR_TOOL_SURFACEOFFSET"));
    a->setStatusTip(QObject::tr("TBR_TOOL_SURFACEOFFSET"));
    connect(a, SIGNAL(triggered()), this, SLOT(onToolAction()));
    myToolActions.insert(ToolSurfaceOffsetId, a);
...
void ApplicationCommonWindow::onImportFile()
{
    QString file_name = QFileDialog::getOpenFileName(this,
        tr("Import IGES"), "F:/Sushou/QianJ/OCC", tr("IGES Files (*.igs)"));
...
void ApplicationCommonWindow::onToolAction()
{
    ...
    if (sentBy == myToolActions.at(ToolAnalyseNormalId))
    {
        doc->onAnalyseNormal();
    }
    if (sentBy == myToolActions.at(ToolSurfaceOffsetId))
    {
        doc->OnSurfaceOffset();
    }
}
