...
TranslateDlg* Translate::getDialog(const int format, const bool import)
{
    ...
    switch (format)
    {
    case FormatBREP: formatfilter = "*.brep"; break;
    case FormatIGES:formatfilter = "*.igs"; break;
    case FormatSTEP: formatfilter = "*.stp"; break;
    case FormatVRML: formatfilter = "*.vrml"; break;
    case FormatSTL: formatfilter = "*.stl"; break;
    default: formatfilter = "*.igs"; break;
    }
    ...
    QString formatFilter = formatfilter;
    QString allFilter = "*.*";
...
Handle(TopTools_HSequenceOfShape) Translate::importIGES(const QString& file)
{
    ...
    if (status == IFSelect_RetDone)
    {
        ...
        // 分别读入
        Standard_Integer num_shape = Reader.NbShapes();
        for (int i = 0; i < num_shape; i++)
        {
            TopoDS_Shape shape = Reader.Shape(i + 1);
            aSequence->Append(shape);
        }
    }
    return aSequence;
